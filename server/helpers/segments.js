...
function buildSegments(course_id){
  ...
  const dogOwnerId = (d)=> d?.eigenaar_id || null;
  ...
  function pushSeg(key, cust, dog, aggregates){
    seg[key].push({
      customer_id: cust?.id || null,
      customer_name: cust?.naam || '',
  ...
  for(const [dog_id, list] of byDog){
    ...
    const cust = custById.get(dogOwnerId(dog));
    seenOwners.add(cust?.id);
    ...
    if(hasApprovedRemainingValid){
      ...
      if(dLeft!==null && dLeft>0 && dLeft<=14){
        pushSeg('nearly_expiring', cust, dog, {remaining_total: remainingTotal, valid_until: maxValidUntil, days_left: dLeft});
      }
      ...
      if(hasPending){
        pushSeg('pending_approval', cust, dog, {remaining_total: remainingTotal, valid_until: maxValidUntil, days_left: dLeft});
      }
    }
  }
  ...
  customers.forEach(c=>{
    ...
    if(dogsOf.length>0){
      ...
      if(!hasAnyForCourse){
        pushSeg('inactive', c, null, {remaining_total: 0, valid_until: null, days_left: null});
      }
    }
  });
