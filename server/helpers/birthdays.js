...
function checkTodayBirthdays(){
  ...
  const birthdayDogs = dogs.filter(d=>{
    if(!d.gebdatum) return false;
    const dt = new Date(d.gebdatum);
    return dt.getDate()===day && (dt.getMonth()+1)===month;
  });

  birthdayDogs.forEach(d=>{
    const cust = customers.find(c=>c.id===d.eigenaar_id);
    if(!cust) return;
    const policy = getPolicy('dog_birthday') || { audience:'customer', delivery:['email'] };
    notify({
      type:'dog_birthday',
      message:`🎂 Vandaag is ${d.naam} jarig!`,
      customer_id:cust.id,
      dog_id:d.id,
