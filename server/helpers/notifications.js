...
function create({ type, message, customer_id, dog_id, audience='customer', delivery=['dashboard'], email_to=null }){
  const all = read();
  const rec = { id: uuid(), type, message, customer_id, dog_id, audience, delivery, email_to,
    seen_by_customer:false, seen_by_admin:false, created_at:new Date().toISOString() };
