...
export async function forwardToSuperhond(payload, meta = {}) {
  ...
  const key = process.env.SUPERHOND_API_KEY || "";
  if (!url) {
  ...
  // Retry met exponentiële backoff (max 5 pogingen: 0.5s, 1s, 2s, 4s, 8s)
  let attempt = 0;
  let lastErr = null;
  while (attempt < 5) {
    try {
...
export async function savePending(payload, meta = {}) {
  ...
  log(`QUEUED ${path.basename(file)} (reason: ${meta.error || "unknown"})`);
  return file;
}

export function startQueueWorker() {
  const interval = Number(process.env.QUEUE_INTERVAL_MS || 15000); // elke 15s
  async function tick() {
    try {
      ...
      for (const fname of files) {
        ...
        if (Date.now() < (rec.nextAttemptAt || 0)) continue; // nog te vroeg
        try {
          ...
          const backoff = Math.min(600000, Math.pow(2, rec.retries) * 1000); // max 10 min
